...
def save_url_classification_results(results, output_file='url_classification_results.json'):
    """URL 분류 결과를 JSON 파일로 저장"""
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"URL 분류 결과가 {output_file}에 저장되었습니다.")


def create_url_classification_report(results, output_file='url_classification_report.json'):
    """URL 분류 결과 보고서 생성"""
    report = {
        'timestamp': datetime.now().isoformat(),
...
def print_url_classification_summary(report):
    """URL 분류 결과 요약 출력"""
    print("\n" + "="*50)
    print("URL 음악 분류 결과 요약")
    print("="*50)
    
    print(f"총 URL 수: {report['total_urls']}")
    print(f"성공한 분류: {report['successful_classifications']}")
    print(f"실패한 분류: {report['failed_classifications']}")
    
    if report['successful_classifications'] > 0:
        success_rate = (report['successful_classifications'] / report['total_urls']) * 100
        print(f"성공률: {success_rate:.1f}%")
